...
export const SearchBox = ()=>
{
  const router = useRouter()
    ...
    return <div className="ltablet:max-w-sm bg-muted-200 dark:bg-muted-800/60 rounded-xl p-8 lg:max-w-md">
    <div className="mb-8 max-w-sm">
      <svg
        ...
        aria-hidden="true"
        role="img"
        className="icon text-primary-500 mb-2 h-10 w-10"
        width="1em"
        height="1em"
        viewBox="0 0 256 256"
      >
      ...
      <p
        className="font-heading text-xl font-medium leading-normal leading-normal mb-2"
      >
        搜索课表
      </p>
    ...
    <div className="relative w-full" >
      <form className="group/autocomplete relative"
      method="GET"
      action="/search"
      >
        <input
          ...
          type="search"
          tabIndex={0}
          className="nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 focus:border-muted-300 focus:shadow-muted-300/50 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 dark:focus:shadow-muted-800/50 peer w-full border bg-white font-sans leading-5 outline-transparent transition-all duration-300 focus:shadow-lg focus:ring-0 disabled:cursor-not-allowed disabled:opacity-75 px-2 h-10 py-2 text-sm leading-5 pe-4 ps-9 placeholder:text-transparent dark:placeholder:!text-transparent rounded-xl"
          placeholder="请输入..."
        />
        <label
