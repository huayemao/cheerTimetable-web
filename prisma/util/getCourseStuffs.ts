import { groupBy, map, compact, omit, isArray } from 'lodash';
...
import { getLocationName } from '../seedCoursesAndLessons';
...
export async function getCourseStuffs(
  ...
  const { jx02id, kcmc: name } = COURSES.find((e) => e.kch?.trim() === subjectId.trim()) || {};
  if (!jx02id) {
  ...
  const canUseClassIdasKey = !!candidateKey.trim() &&
    /^\w+$/.test(candidateKey) &&
  ...
  const courses = map(
    ...
    (v: (LessonRes1 & { term: string; })[], k): Course => {
      ...
      return {
        ...
        subjectId,
        term: v[0].term,
        className: classNameFullValue && classNameFullValue.length > 60
  ...
  const getTeacherIdbyjg0101id = (id) => TEACHERS.find((t) => t.jg0101id === id)?.jgxm.match(/\[(.+)\]/)?.[1] ||
    'unknown';

  const lessons = map(record, (v: LessonRes[], k): any[] => v.map(({ 上课地点, 开课编号, 单双周, 开课时间, 上课周次, 授课教师 }) => {
  ...
  const tuitions = lessons.flatMap((l) => l.teacherIds.map(
    (id): Tuition => ({
      lessonId: l.id,
      teacherId: id,
    })
  ...
  function getCourseId(v: (LessonRes1 & { term: string; })[], k) {
    ...
    if (candidateId && /^\w+$/.test(candidateId)) {
      ...
      if (items.find((e) => e.开课编号 === composited) &&
