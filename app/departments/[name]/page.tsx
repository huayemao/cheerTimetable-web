...
const data = await getProfessions()
  const params = Object.values(data).flat().map(
    ((e) => {
      return {
...
const Profession = ({data}:{data:{
  ...
  return <div className="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md hover:shadow-muted-300/30 dark:hover:shadow-muted-800/30 hover:shadow-xl group-hover:!border-primary-500 p-5">
  <div className="mb-6 flex gap-2">
    <div
      className="relative inline-flex shrink-0 items-center justify-center outline-none h-10 w-10 rounded-full bg-muted-100 dark:bg-muted-700"
    >
      <div className={clsx("bg-info-500/20 text-info-500 flex h-full w-full items-center justify-center overflow-hidden text-center transition-all duration-300 rounded-full", {
    ...
    <div>
      <p
        className="font-heading text-sm font-medium leading-normal line-clamp-1"
      >
        {name}
      </p>
  ...
  <div className="flex items-center justify-between text-muted-400">
    ...
    <div className="flex items-center gap-4">
      <div className="flex items-center gap-1 text-sm">
        ...
        <span className="font-sans">{studentCount}</span>
      </div>
    </div>
  </div>
